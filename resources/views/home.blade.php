...
<section id="service" class="border-y-2 border-ink bg-cream px-5 py-20 lg:px-8"><div class="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2"><div><p class="eyebrow">Sykkelservice</p><h2>Enkel service og vedlikehold</h2><p class="lead">Har du allerede en sykkel som trenger litt omsorg? Jeg tilbyr enkelt vedlikehold og reparasjoner av vanlige sykler (tar imot én sykkel per uke).</p><div class="mt-8"><a href="{{ route('service.create') }}" class="inline-block rounded-full bg-rust px-7 py-4 font-extrabold text-white">Bestill service her →</a></div></div><ul class="card grid gap-3 sm:grid-cols-2">@foreach(['Justering av gir','Justering av bremser','Kjede, kassett og klosser','Dekk og slanger','Montering av utstyr','Rengjøring av drivverk','Sesongklargjøring','Generell gjennomgang'] as $item)<li>✓ {{ $item }}</li>@endforeach</ul></div></section>
