...
<header class="border-b-2 border-ink bg-paper"><div class="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8"><a href="{{ route('home') }}" class="focus-ring flex items-center gap-3"><span class="grid size-11 place-items-center rounded-full border-2 border-ink bg-sun text-2xl">⚙</span><span><strong class="block font-display text-lg uppercase leading-none">Han Sykkeltypen</strong><small class="mt-1 block font-bold uppercase tracking-[.18em] text-moss">Brukt. Fikset. Klar.</small></span></a><nav class="hidden items-center gap-7 text-sm font-bold md:flex">@if($hasBikesForSale ?? true)<a href="{{ route('bikes.index') }}">Sykler</a>@endif<a href="{{ route('pricing') }}">Priser</a><a href="{{ route('service.create') }}">Bestill service</a><a href="{{ route('home') }}#henting">Henting</a><a href="{{ route('home') }}#om">Om</a><a class="rounded-full bg-rust px-5 py-3 text-white" href="{{ route('home') }}#kontakt">Ta kontakt</a></nav><a class="rounded-full bg-rust px-4 py-2.5 text-sm font-bold text-white md:hidden" href="{{ route('home') }}#kontakt">Kontakt</a></div></header>
@if(session('status'))<div id="toast-status" role="status" aria-live="polite" class="fixed right-5 top-5 z-50 max-w-sm rounded-3xl border-2 border-ink bg-sun p-4 shadow-hard"><div class="flex items-start gap-3"><span class="mt-0.5 grid size-7 shrink-0 place-items-center rounded-full border-2 border-ink bg-paper font-bold">✓</span><p class="font-bold leading-snug">{{ session('status') }}</p><button type="button" class="ml-2 -mt-1 rounded-full px-2 py-1 font-bold" aria-label="Lukk" onclick="document.getElementById('toast-status')?.remove()">×</button></div></div><script>setTimeout(()=>document.getElementById('toast-status')?.remove(),6000);</script>@endif
    ...
    <div class="mx-auto flex max-w-7xl flex-col gap-8 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div class="grid grid-cols-2 gap-8 sm:flex sm:gap-12">
            <nav class="flex flex-col gap-2 text-sm">
                ...
                @if($hasBikesForSale ?? true)<a href="{{ route('bikes.index') }}" class="font-bold text-sun">Sykler til salgs</a>@endif
                <a href="{{ route('pricing') }}" class="hover:text-sun">Priser</a>
